...
$|\mathrm{TFD}(\beta)\rangle = Z^{-1/2} \sum_n e^{-\beta E_n/2} |n\rangle_L |n\rangle_R^*$ over the full
...
Everything is computed in Schmidt form: the $D \times D$ doubled space (41M for n=4,
1.9B for n=5) is never formed. $\langle q_L q_R \rangle = \sum_{nm} \sqrt{p_n p_m} (q_{nm})^2$
...
- **(i) $\rho_L = \rho_\beta$: PASS.** Schmidt weights reproduce the T7a Gibbs
  distribution; cross-check vs t7a_results.json: $\max|dS| = 3.6\mathrm{e}{-15}/1.4\mathrm{e}{-14}$,
  $\max|dq^2| = 0$, $\max|d\mathrm{Areas}| = 0$.
- **(ii) $\beta \to \infty$ limit: CORRECTED, not as spec'd.** The Gibbs-TFD flows to
  the Fock vacuum product ($\beta=10$: $p_{\mathrm{vac}} = 0.9996/0.9995$, $S \to 0.004/0.005$),
...
- **(iii) $S(\beta)$ matches thermal entropy: PASS** (same numbers as (i);
  $S = 8.77/10.69$ at $\beta=0$ $= \log D$, $\to \sim 0$ at $\beta=10$).
...
$|\langle q_L q_R \rangle|$ vs $T$: log-log local slopes drift $0.2 \to 21.7$ (both n); formal
fits give alpha_all $\sim 5.8$ (R2 $\sim 0.69$), alpha_trust($\beta \geq 1$) $\sim 12.0$ (R2 $\sim 0.93$)
— the R2 is deceptive (4 points of an exponential); the drifting slopes rule
out a power law. Instead $d \ln|\mathrm{corr}|/d\beta = -3.008 \approx -3$ on $[5,10]$:
Boltzmann-exponential onset $\sim e^{-3\beta}$, because $q$ needs edges 0,1,2
occupied (leading $E=3$ sector). Honest verdict: thermal onset is exponential,
qualitatively distinct from T5b's $V \sim \epsilon^{0.5}$ sqrt law — complexification
and thermalization are different deformations with different universality
(or lack thereof). Cutoff caveat from T7a stands: $\beta \leq 0.5$ qualitative,
$\beta \geq 1$ quantitative.
