...
fixed n=5, K=8. Swept reference magnetization M=(Na−Nb)/2 at fixed K.
...
| ref | M | sign-agree | max\|q\| | mean\|q\| | Pearson\|q\| vs M=0 spread |
|---|---|---|---|---|---|
| all-a | +4 | — (all q≈0) | 8.7e-18 | 2.8e-18 | −0.12 |
...
| | −2 | 0.60 | 2.50e-02 | 8.76e-03 | +0.14 |
| | −3 | 0.50 | 1.32e-02 | 4.78e-03 | +0.01 |
| all-b | −4 | — (all q≈0) | 1.4e-17 | 2.6e-18 | −0.24 |

## Secondary observations

1. **Polarized endpoints freeze:** M=±4 (all-a / all-b) give q≈1e-17
   (numerical zero) on all triples — confirms the N_b=0 / N_a=0
   spin-freezing mechanism in this engine.
2. **a↔b mirror symmetry exact:** M=+k and M=−k give byte-identical q
   vectors (J_i·J_j is invariant under a↔b), as expected.
3. **Spatial control at M=0:** concentrating all b-bosons on one edge
   ...
   +0.86) but rescales mean|q| ×3 (7.4e-3 → 2.25e-2).
4. **Across-M decoherence:** |q|-pattern correlation vs the M=0 baseline
   decays 1.0 → 0.24 → 0.14 → 0.01 with growing |M| — which triple is
   loudest is M-sector-dependent.
5. **Magnitude is non-monotonic in |M|**, peaking at M=0 concentrated.

## Red-team check

Recomputed q_123 for M=0_spread via the explicit commutator
q=i[A12,A23] expectation: 0.003811345194626947 (imag 2.5e-17),
matching the −2Im route (0.0038113451946269385) and the run log
