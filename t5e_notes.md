...
$q = -2 \mathrm{Im}\langle A_{01} \psi | A_{12} \psi \rangle$, $V = \gamma^{1.5} \sqrt{|q|}$.

## Verdict

**The $K^{1.5}$ law does NOT hold.** No family shows alpha $\sim 1.5$:

1. **Vertex-scaled family (b-bosons loaded onto the measured triple,
   $K = 4+3s$): $\langle q \rangle$ is EXACTLY linear in $s$** — $q/s = -1.059408\mathrm{e}{-3}$ at
   all five points ($K=10..22$, equal to 9 digits) plus the $K=7$
   regression point. Hence $V \sim (K-4)^{0.5}$, i.e. alpha $\to 0.5$
   ...
   shifted square root; local slopes decline $+0.77 \to +0.62$ toward 0.5).
...
2. **Uniform $M=0$ family (fixed shape, $K=8..24$, seeds 11 and 42):
   $q = 0$ to solver precision** ($|q| \leq 9\mathrm{e}{-13}$, signs fluctuate, both
   engines agree to $\sim 1\mathrm{e}{-17}$ at $K=8$). The t5e_results.json uniform fits
   (alpha $\sim +3.9$) fit SOLVER NOISE, not physics — do not quote them.
   Uniform scaling of the coherent state develops NO volume at all.

Classical $V \sim r^3$ would need $\langle q \rangle \sim K^3$. Observed: $\langle q \rangle \sim K^1$ (triple
loading) or $\langle q \rangle \sim 0$ (uniform scaling). Per the probe guide this is the
...
- Single engine: Rust on-the-fly (no stored operators), validated vs
  the stored engine at $K=8,12$ (abs-diff $\sim 1\mathrm{e}{-17}..1\mathrm{e}{-15}$, both consistent
  with zero there). Real-plane (eps=0) controls give EXACTLY 0 at every
  $K$ tested. Closure sum $\langle n_i \rangle = K$ exact. Sanity gate: $K=7$ vertex $V =
  3.767\mathrm{e}{-3}$, in the t5b $1\mathrm{e}{-3}..1\mathrm{e}{-2}$ range. Seed-42 uniform points
  confirm the zero.
...
1. **Truncation saga.** The first recorded run (7a8364f6) used Taylor
   ...
   (uniform alpha $\sim -2$, vertex alpha $\sim +3.2$). The audit child proved
...
2. **Uniform zero is an upper bound, not a theorem.** $|q| \leq \sim 1\mathrm{e}{-12}$ at
   $K \leq 24$ given tol $1\mathrm{e}{-13}$ states. Whether it is exactly zero (a new
   selection rule for balanced references?) or merely tiny needs higher
   precision or an analytic argument — open follow-up.
3. **Narrow lever arm.** $K=8..24$ is 0.5 dex (Fock dim $1\mathrm{e}{4}..1\mathrm{e}{7}$ at n=4);
   $K=28+$ needs $\sim 30$M-dim vectors, beyond this 8 GB machine.
