...
Minkowski-ADJACENT triples even though all-triples sign-agreement is $\sim 0.5$?
...
all $\approx 0.589$, local $\approx 0.594$ — dead even. n=4: exact 0.500 in all 3 channels
...
2. n=4 CM 2->2 normals are planar (rank 2): no 3D polyhedron exists, and
   ...
   verdict only. Genuine subsetting needs $n \geq 5$.
...
- n=4 Rust-test replication: mine -8.276875e-4, expm -8.276872e-4
  ...
  each other only through shared truncation. Convergence needs $\sim 25$ terms.
- n=3 cross-check: expm gives $q \sim 4\mathrm{e}{-17}$ (exact zero). Reason: EVERY n=3
