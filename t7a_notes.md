...
(B) Perelomov-weighted diagonal `w ∝ |$c_{n}$|² e^{-beta E}` (plane-dependent);
(C) thermally-rescaled pure `|psi_beta> ∝ $c_{n}$ e^{-beta E/2} |n>` (TFD precursor).

## Results (run 0551787b, full tables in t7a_results.json)

1. **Thermal areas are nonzero.** Gibbs: uniform across edges (omega uniform),
   0.369 (n=4) / 0.346 (n=5) per edge at beta=0 $\to$ ~2e-5 at beta=10.
...
3. **Fluctuations are nonzero — this is where the thermal volume info lives.**
   Gibbs `Tr(rho q²)`: 0.357/0.349 (beta=0) $\to$ 7e-14 (beta=10).
...
Boundary weight P(E=K): 0.53/0.56 at beta=0 $\to$ 1e-27/1e-31 at beta=10.
Z_capped/$Z_{inf}$: 2e-5/1e-6 at beta=0.1 (cutoff bites) $\to$ 0.86/0.82 at beta=1 $\to$
1.000 at beta $\geq$ 2. Trust beta ≳ 1 quantitatively; beta $\leq$ 0.5 is qualitative.
Gibbs entropy S: 8.77/10.69 (beta=0) $\to$ ~0 (beta=10); recorded for T7b
