...
- T7: Thermofield-double construction — double the Schwinger system via TFD purification to get thermal volume on real momenta (no complexification). 🔄 PROPOSED 2026-09-20. Key advantage: H = sum omega_i (n_a,i+n_b,i) is occupation-diagonal, so e^{-βH/2} rescales Fock amplitudes exactly — no Taylor, no truncation, exact at any K. **Load-bearing order: single-copy first.** T7a (single-copy thermal state: thermal areas Tr(ρA_i) nonzero, but Tr(ρq)=0 EXACTLY since ρ is Fock-diagonal and every real Fock state has <n|q|n>=0 — the signal lives in Tr(ρq²) fluctuations); T7b (TFD construction, verify ρ_L=ρ_β and S(β)=thermal entropy); T7c (two-sided correlator <q_L q_R>(β) vs Tr(ρq²) and S(β) — motivated by T7a's null mean); T7d (thermal scaling <q_L q_R>~T^α vs T5b's √ε); T7e (complexified momenta in TFD — explicitly NOT subsumed, open question, separate protocol). Spec: `implementation-details/thermofield-double-volume.md`. Aspirational context (not the focus): candidate concrete LQG wormhole state.
